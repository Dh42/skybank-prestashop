...

{
...

	{
		...
		$this->tab_name[1] = 'Subscription Orders';
		...
		$this->description = $this->l('Accept Credit, Debit, and ACH Payments Online With An Optional Subscription Feature Today!');
...
Configuration::updateValue('SKYBANK_AIM_CHECK_OS', _PS_OS_ERROR_);
...

	{
		...
		Configuration::deleteByName('SKYBANK_AIM_DISPLAY_OPEN');
...

	{
...

		{
			...
			Configuration::updateValue('SKYBANK_AIM_DISPLAY_OPEN', Tools::getvalue('skybankaim_display_open'));
		...
		$this->context->smarty->assign(array(
			...
			'SKYBANK_AIM_DISPLAY_OPEN' => (int)Configuration::get('SKYBANK_AIM_DISPLAY_OPEN'),
...

	{
...

		{
			...
			$this->context->smarty->assign('skybankaim_display_open', (bool)Configuration::get('SKYBANK_AIM_DISPLAY_OPEN'));
...

	{
		...
		$params = array(
			...
			'Amount' => Tools::getValue('captureamt') ? Tools::getValue('captureamt') : '',
...

		{
			...
			<div class="col-lg-7">
				...
				<div class="panel">
					...
					<form action="" method="POST" class="defaultForm form-horizontal" id="skybankform">
						...
						if (strstr($order_payment[0]->transaction_id, 'Auth') || (isset($canvoid) && $canvoid)) {
							...
							$html .= '
								...
								<div class="col-lg-2" style="width:413px">

									<input type="submit" onclick="return confirm(\'Do you wish to void this transaction?\');return false;" class="btn btn-default" name="submitCancel" value="'.$this->l('Void').'"'.(strstr($order_payment[0]->transaction_id, 'Auth') ? 'disabled' : '').'/>



									'.(strstr($order_payment[0]->transaction_id, 'Auth') ? '
										<div class="col-lg-5">
											<input type="text" name="captureamt" value="" placeholder="Amount to capture (leave blank for the whole amount)">
										</div>
										<input type="submit" class="btn btn-default" name="SubmitForce" value="Capture Transaction"/>' : '').'
